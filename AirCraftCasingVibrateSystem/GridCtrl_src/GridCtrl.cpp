...
// erase the background normally through OnEraseBkgnd, and didn't fill the GCMemDC's
...
CImageList* CGridCtrl::CreateDragImage(CPoint *pHotSpot)
{
    ...
    *pHotSpot = rect.BottomRight(); 
    ...
    CDC GCMemDC;
    CBitmap bm;
    GCMemDC.CreateCompatibleDC(pDC);
    bm.CreateCompatibleBitmap(pDC, rect.Width(), rect.Height());
    CBitmap* pOldBitmap = GCMemDC.SelectObject(&bm);
    GCMemDC.SetWindowOrg(0, 0);
    
    // Draw cell onto bitmap in GCMemDC
    CGridCellBase* pCell = GetCell(cell.row, cell.col);
    if (pCell)
        pCell->Draw(&GCMemDC, cell.row, cell.col, rect, FALSE);
    
    // Clean up
    GCMemDC.SelectObject(pOldBitmap);
    ReleaseDC(pDC);
