...
CChartLogarithmicAxis::CChartLogarithmicAxis()
 : CChartAxisDu(), m_dFirstTickValue(1)
{
}
...
TChartString CChartLogarithmicAxis::GetTickLabel(double TickValue)
{
	double fLogDecCount; 
	int    nLogDecCount; 
	...
	// 当fLogDecCount为负数时，
	// 原来是nLogDecCount = (int)(fabs(fLogDecCount) + 0.1);
	// 自己改成nLogDecCount = (int)(fabs(floor(fLogDecCount)));

	if (fLogDecCount < 0.0)
		nLogDecCount = (int)(fabs(floor(fLogDecCount))); 
	else 
		nLogDecCount = 0; 
...
long CChartLogarithmicAxis::ValueToScreenDiscrete(double Value)
{
...
{
	// 原来是(int)log10(m_MinValue);
	// 自己改成(int)floor(log10(m_MinValue));
	int LogBase = (int)floor(log10(m_MinValue));
