...
void CChartGrid::Draw(CDC *pDC, int nSerieIndex)
{
	if (!m_bIsVisible)
		return;
	if (!pDC->GetSafeHdc() )
	...
	CRect plottingRect;
	switch (pDuChartCtrl->m_shuxing.m_nHebingOrFengkai)
	{
	case CDuChartCtrlShuxing::typeHebing:
		plottingRect = m_pParentCtrl->GetPlottingRect();
		break;
	case CDuChartCtrlShuxing::typeFengkai:
	case CDuChartCtrlShuxing::typeGongyongXZhou:
		plottingRect = m_pParentCtrl->GetPlottingRectFromSerieIndexDu(nSerieIndex);
		break;
	}
