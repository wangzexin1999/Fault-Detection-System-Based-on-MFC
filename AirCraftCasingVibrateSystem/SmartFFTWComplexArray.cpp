...
void SmartFFTWComplexArray::push_back(fftw_complex fftwComplex){
	if (m_fftwComplexArray.index >= m_fftwComplexArray.size){
		...
		fftw_complex *dintemp = (fftw_complex*)fftw_malloc(sizeof(fftw_complex)* this->m_fftwComplexArray .size);
		memcpy(dintemp, m_fftwComplexArray .data, sizeof(fftw_complex)*m_fftwComplexArray .size);
		////让指针数据指向新创建的内容
		m_fftwComplexArray .data = dintemp;
	}
